...
from e8_self_teacher_v2 import solve_task as self_teach_solve
...
class FusedSubstrateEngine:
    ...
    def __init__(self, store_path: str = None):
        ...
        # === E8 Concept Embeddings ===
        import sqlite3
        e8_db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                   "schema_engine", "schema.db")
        self._e8_db = None
        try:
            if os.path.exists(e8_db_path):
                self._e8_db = sqlite3.connect(e8_db_path, check_same_thread=False)
                count = self._e8_db.execute(
                    "SELECT COUNT(*) FROM e8_concepts"
                ).fetchone()[0]
                print(f"  E8 concepts: {count} loaded from {e8_db_path}")
            else:
                print(f"  E8 concepts: schema.db not found at {e8_db_path}")
        except Exception as e:
            print(f"  E8 concepts: init error: {e}")

    
    def process(self, model: str, prompt: str, system: str = None,
                stream: bool = False, approach_vector: list = None,
                bypass_static: bool = False) -> Dict:
        """Process a request through the substrate + codebook pipeline.
        approach_vector: optional geometric bias from mother.
        bypass_static: skip static codebook, force pure E8 field processing."""
        ...
        # Mix approach_vector into signal (mother-worker corridor bias)
        if approach_vector is not None:
            
            av = np.array(approach_vector[:len(signal)], dtype=signal.dtype)
            if len(av) < len(signal):
                av = np.pad(av, (0, len(signal) - len(av)))
            # Additive bias: preserves task geometry, shifts exploration corridor
            norm = float(np.linalg.norm(av))
            if norm > 1e-8:
                av = av / norm * float(np.linalg.norm(signal)) * 0.3  # 30% bias strength
                signal = signal + av
    ...
    def _decode_response(self, model: str, role: CoreRole, task: dict,
                         ...
                         system: str, bypass_static: bool = False) -> tuple:
        """
        Three-tier decoding:
        ...
        if bypass_static:
            pass  # Pure E8 field mode - skip all static templates
        elif model in ('analyst',):
        ...
        # === Tier 2.5: Self-teaching synthesis ===
        try:
            synth_code = self_teach_solve(task)
            if synth_code:
                self.self_teach_hits += 1
                # Feed to dynamic codebook for future recall
                task_id = task.get("task_id", "self_teach")
                try:
                    self.dynamic.learn(task, synth_code, task_id=f"self_teach_{task_id}")
                except Exception:
                    pass
                if model in ("coder", "solver"):
                    synth_code = self._format_code(synth_code, prompt)
                    return synth_code, "self_teach"
                elif model in ("analyst",):
                    return "Transformation synthesized from geometric observation of training pairs.", "self_teach"
                else:
                    return "Pattern synthesized. Code available.", "self_teach"
        except Exception as e:
    ...
    def _text_to_signal(self, prompt: str, system: str = None) -> np.ndarray:
        """Convert text to substrate signal using E8 concept embeddings.
        
        Words found in e8_concepts table get their 240D standing wave signature.
        Unknown words fall back to byte encoding.
        The 240D E8 signatures are projected into the 1024D signal space.
        """
        text = (system or "") + " " + prompt
        signal = np.zeros(1024, dtype=np.float32)
        
        # Try E8 concept lookup for each word
        words = text.lower().split()
        e8_contributions = 0
        
        if hasattr(self, '_e8_db') and self._e8_db is not None:
            for wi, word in enumerate(words):
                clean = ''.join(c for c in word if c.isalpha())
                if not clean:
                    continue
                try:
                    row = self._e8_db.execute(
                        "SELECT embedding FROM e8_concepts WHERE word = ?", (clean,)
                    ).fetchone()
                    if row:
                        # Unpack 240D signature (240 x float32 = 960 bytes)
                        e8_sig = np.frombuffer(row[0], dtype=np.float32)
                        # Project 240D into 1024D signal space
                        # Each word occupies a shifted window to preserve ordering
                        offset = (wi * 4) % 784  # shift per word position, wrap at 1024-240
                        signal[offset:offset+240] += e8_sig
                        e8_contributions += 1
                except Exception:
                    pass
        
        # Fallback: byte encode any remaining signal gap
        if e8_contributions == 0:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "self_teach_hits": self.self_teach_hits,
            "hit_rate": (
                (self.static_hits + self.dynamic_hits + self.self_teach_hits) / max(self.request_count, 1)
            ),
    ...
    def do_POST(self):
        ...
        if self.path == '/api/generate':
            result = self.server.engine.process(
                ...
                approach_vector=data.get('approach_vector'),
                bypass_static=data.get('bypass_static', False),
            )
            self._json_response(result)
