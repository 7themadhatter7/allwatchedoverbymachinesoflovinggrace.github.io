...
class GridEncoder:
    ...
    def encode_task(task: Dict) -> np.ndarray:
        """
        Encode full ARC task with rich semantic features.
        
        Enhanced encoder: instead of averaging pair signals and crushing
        with consistency weighting, builds task-level semantic signal that
        exposes structural differences for council differentiation.
        
        Band 7 (768-895): Task-level semantic features
          768-775: Dimension change signature
          776-799: Object topology (counts, sizes, holes, dividers)
          800-831: Spatial relationship features
          832-863: Cross-pair variance signature
          864-895: Task meta-features
        Band 8 (896-959): Variance fingerprint
          960-1023: Reduced raw hash
        """
        train = task.get('train', [])
        if not train:
            return np.zeros(GridEncoder.SIGNAL_SIZE, dtype=np.float32)
        
        # Get base pair signals
        pair_signals = []
        for pair in train:
            sig = GridEncoder.encode_pair(pair['input'], pair['output'])
            pair_signals.append(sig)
        
        # Average for consensus (bands 0-767)
        composite = np.mean(pair_signals, axis=0).astype(np.float32)
        
        # NO consistency weighting — preserve signal strength
        # Variance goes into dedicated bands instead
        
        # ═══ BAND 7 (768-895): Task-level semantic features ═══
        
        # --- Dimension change signature (768-775) ---
        dim_feats = []
        for pair in train:
            ig = np.array(pair['input']); og = np.array(pair['output'])
            ih, iw = ig.shape; oh, ow = og.shape
            f = np.zeros(8)
            f[0] = oh / max(ih, 1)
            f[1] = ow / max(iw, 1)
            f[2] = 1.0 if oh == ih and ow == iw else 0.0
            f[3] = 1.0 if oh < ih else 0.0
            f[4] = 1.0 if oh > ih else 0.0
            f[5] = 1.0 if oh * ow < ih * iw else 0.0
            f[6] = 1.0 if oh * ow > ih * iw else 0.0
            f[7] = abs(oh * ow - ih * iw) / max(ih * iw, 1)
            dim_feats.append(f)
        composite[768:776] = np.mean(dim_feats, axis=0)
        
        # --- Object topology (776-799) ---
        obj_feats = []
        for pair in train:
            ig = np.array(pair['input']); og = np.array(pair['output'])
            f = np.zeros(24)
            ...
            in_mask = ig != 0
            in_labeled, in_nobj = ndimage.label(in_mask)
            in_colors = set(ig[in_mask].astype(int).tolist()) if in_mask.any() else set()
            
            out_mask = og != 0
            out_labeled, out_nobj = ndimage.label(out_mask)
            out_colors = set(og[out_mask].astype(int).tolist()) if out_mask.any() else set()
            
            f[0] = in_nobj / 30.0
            f[1] = out_nobj / 30.0
            f[2] = (out_nobj - in_nobj) / 30.0
            f[3] = len(in_colors) / 10.0
            f[4] = len(out_colors) / 10.0
            f[5] = len(out_colors - in_colors) / 10.0
            f[6] = len(in_colors - out_colors) / 10.0
            f[7] = len(in_colors & out_colors) / max(len(in_colors | out_colors), 1)
            
            if in_nobj > 0:
                in_sizes = sorted([np.sum(in_labeled == i) for i in range(1, in_nobj + 1)], reverse=True)
                f[8] = in_sizes[0] / max(ig.size, 1)
                f[9] = in_sizes[-1] / max(ig.size, 1)
                f[10] = np.std(in_sizes) / max(np.mean(in_sizes), 1)
            
            if out_nobj > 0:
                out_sizes = sorted([np.sum(out_labeled == i) for i in range(1, out_nobj + 1)], reverse=True)
                f[11] = out_sizes[0] / max(og.size, 1)
                f[12] = out_sizes[-1] / max(og.size, 1)
                f[13] = np.std(out_sizes) / max(np.mean(out_sizes), 1)
            
            in_bg = np.sum(ig == 0) / max(ig.size, 1)
            out_bg = np.sum(og == 0) / max(og.size, 1)
            f[14] = in_bg
            f[15] = out_bg
            f[16] = out_bg - in_bg
            
            # Enclosure detection
            if ig.shape[0] > 2 and ig.shape[1] > 2:
                in_zeros = (ig == 0).astype(int)
                in_z_labeled, in_z_n = ndimage.label(in_zeros)
                in_holes = 0
                for i in range(1, in_z_n + 1):
                    rows, cols = np.where(in_z_labeled == i)
                    if rows.min() > 0 and rows.max() < ig.shape[0]-1 and cols.min() > 0 and cols.max() < ig.shape[1]-1:
                        in_holes += 1
                f[17] = in_holes / 10.0
            
            if og.shape[0] > 2 and og.shape[1] > 2:
                out_zeros = (og == 0).astype(int)
                out_z_labeled, out_z_n = ndimage.label(out_zeros)
                out_holes = 0
                for i in range(1, out_z_n + 1):
                    rows, cols = np.where(out_z_labeled == i)
                    if rows.min() > 0 and rows.max() < og.shape[0]-1 and cols.min() > 0 and cols.max() < og.shape[1]-1:
                        out_holes += 1
                f[18] = out_holes / 10.0
            
            # Divider detection
            ih, iw = ig.shape
            h_div = sum(1 for r in range(ih) if len(set(ig[r,:].astype(int).tolist())) == 1 and ig[r,0] != 0)
            v_div = sum(1 for c in range(iw) if len(set(ig[:,c].astype(int).tolist())) == 1 and ig[0,c] != 0)
            f[19] = h_div / max(ih, 1)
            f[20] = v_div / max(iw, 1)
            f[21] = 1.0 if h_div > 0 or v_div > 0 else 0.0
            
            f[22] = len(set(ig.flatten().astype(int).tolist())) / 10.0
            f[23] = len(set(og.flatten().astype(int).tolist())) / 10.0
            
            obj_feats.append(f)
        composite[776:800] = np.mean(obj_feats, axis=0)
        
        # --- Spatial relationship features (800-831) ---
        spatial_feats = []
        for pair in train:
            ig = np.array(pair['input']); og = np.array(pair['output'])
            ih, iw = ig.shape; oh, ow = og.shape
            ...
            if ih == oh and iw == ow:
                ...
                if diff.any():
                    ...
                    f[1] = rows.mean() / max(ih, 1)
                    f[2] = cols.mean() / max(iw, 1)
                    f[3] = rows.std() / max(ih, 1)
                    f[4] = cols.std() / max(iw, 1)
                    
                    edge = np.zeros_like(diff)
                    edge[0,:] = True; edge[-1,:] = True; edge[:,0] = True; edge[:,-1] = True
                    f[5] = np.sum(diff & edge) / max(np.sum(diff), 1)
                    f[6] = np.sum(diff & ~edge) / max(np.sum(diff), 1)
                    
                    for c in range(10):
                        in_c = (ig == c); out_c = (og == c)
                        if in_c.any() or out_c.any():
                            gained = np.sum(out_c & ~in_c)
                            lost = np.sum(in_c & ~out_c)
                            f[7 + c] = (gained - lost) / max(ig.size, 1)
                    
                    row_changes = diff.any(axis=1)
                    col_changes = diff.any(axis=0)
                    f[17] = np.mean(row_changes)
                    f[18] = np.mean(col_changes)
                    
                    change_labeled, change_n = ndimage.label(diff.astype(int))
                    f[19] = change_n / 30.0
                    
                    if ih > 1:
                        f[20] = np.mean(diff == diff[::-1,:])
                    if iw > 1:
                        f[21] = np.mean(diff == diff[:,::-1])
            else:
                f[22] = 1.0
                if oh <= ih and ow <= iw:
                    f[23] = 1.0
                    ...
                    f[24] = 1.0
                if ih > 0 and iw > 0:
                    f[25] = 1.0 if oh % ih == 0 and ow % iw == 0 else 0.0
                    f[26] = 1.0 if oh > 0 and ow > 0 and ih % oh == 0 and iw % ow == 0 else 0.0
                if oh <= ih and ow <= iw:
                    found = False
                    for r in range(ih - oh + 1):
                        for c in range(iw - ow + 1):
                            if np.array_equal(ig[r:r+oh, c:c+ow], og):
                                f[27] = 1.0
                                f[28] = r / max(ih, 1)
                                f[29] = c / max(iw, 1)
                                found = True; break
        ...
        composite[800:832] = np.mean(spatial_feats, axis=0)
        
        # --- Cross-pair variance signature (832-863) ---
        if len(pair_signals) > 1:
            var = np.var(pair_signals, axis=0)
            composite[832] = np.mean(var[0:128])
            composite[833] = np.mean(var[128:256])
            composite[834] = np.mean(var[256:384])
            composite[835] = np.mean(var[384:512])
            composite[836] = np.mean(var[512:640])
            composite[837] = np.mean(var[640:768])
            composite[838] = np.mean(var[768:896])
            
            composite[839] = np.mean(var)
            composite[840] = np.median(var)
            composite[841] = np.max(var)
            
            band_vars = [np.mean(var[i*128:(i+1)*128]) for i in range(8)]
            max_band = np.argmax(band_vars)
            composite[842 + max_band] = 1.0
            
            shapes = [(np.array(p['input']).shape, np.array(p['output']).shape) for p in train]
            in_shapes = set(s[0] for s in shapes)
            out_shapes = set(s[1] for s in shapes)
            composite[850] = 1.0 if len(in_shapes) == 1 else 0.0
            composite[851] = 1.0 if len(out_shapes) == 1 else 0.0
            composite[852] = 1.0 if all(s[0] == s[1] for s in shapes) else 0.0
        
        # --- Task meta-features (864-895) ---
        composite[864] = len(train) / 10.0
        
        in_areas = [np.array(p['input']).size for p in train]
        composite[865] = np.mean(in_areas) / 900.0
        composite[866] = np.std(in_areas) / 900.0 if len(in_areas) > 1 else 0
        
        out_areas = [np.array(p['output']).size for p in train]
        composite[867] = np.mean(out_areas) / 900.0
        composite[868] = np.std(out_areas) / 900.0 if len(out_areas) > 1 else 0
        
        all_in_colors = set()
        all_out_colors = set()
        for p in train:
            all_in_colors.update(np.array(p['input']).flatten().astype(int).tolist())
            all_out_colors.update(np.array(p['output']).flatten().astype(int).tolist())
        composite[869] = len(all_in_colors) / 10.0
        composite[870] = len(all_out_colors) / 10.0
        composite[871] = len(all_out_colors - all_in_colors) / 10.0
        composite[872] = len(all_in_colors - all_out_colors) / 10.0
        
        test = task.get('test', [])
        if test:
            tg = np.array(test[0]['input'])
            composite[873] = tg.shape[0] / 30.0
            composite[874] = tg.shape[1] / 30.0
            composite[875] = len(set(tg.flatten().astype(int).tolist())) / 10.0
            train_shapes = [np.array(p['input']).shape for p in train]
            composite[876] = 1.0 if tg.shape in train_shapes else 0.0
        ...
        if len(pair_signals) > 1:
            ...
            for i in range(64):
                ...
                composite[896 + i] = np.mean(var[start:end])
