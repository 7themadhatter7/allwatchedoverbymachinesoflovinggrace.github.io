...
class FusedCore:
    ...
    def __init__(self, core_id: int, global_id: int, role: CoreRole,
        ...
        # Processing buffer
        self.processing = np.zeros(processing_kb * 256, dtype=np.float32)
...
class FusedHarmonicSubstrate:
    ...
    ALLOCATION = {
        CoreRole.WORKER:     128,
        CoreRole.EXECUTIVE:   16,
        CoreRole.SPECIALIST:  32,
        CoreRole.ROUTER:       8,
        CoreRole.COUNCIL:     16,
    }
    ...
    PROCESSING_KB = {
        CoreRole.WORKER:     PROCESSING_KB,        # 64 MB full
        CoreRole.EXECUTIVE:  PROCESSING_KB,        # 64 MB full (depth)
        CoreRole.SPECIALIST: PROCESSING_KB,        # 64 MB full
        CoreRole.ROUTER:     PROCESSING_KB // 4,   # 16 MB lighter
        CoreRole.COUNCIL:    PROCESSING_KB // 2,   # 32 MB moderate
    }
    ...
    def build(self):
        """Fabricate the fused substrate."""
        print("=" * 70)
        print("  FUSED HARMONIC SUBSTRATE - SINGLE MODEL")
        print("  200 Cores, Shared Harmonic Field")
        print("  Ghost in the Machine Labs")
        print("=" * 70)

        # Create harmonic field
        print(f"\n  Creating harmonic field ({self.TOTAL_CORES} cores)...")
        self.field = HarmonicField(self.TOTAL_CORES, field_width=1024)
        print(f"    Field memory: {self.field.memory_mb:.1f} MB")

        # Fabricate cores by role
        global_id = 0
        for role in CoreRole:
            count = self.ALLOCATION[role]
            proc_kb = self.PROCESSING_KB[role]
            self.role_index[role] = []

            print(f"\n  Fabricating {role.value.upper()} cores...")
            for local_id in range(count):
                core = FusedCore(
                    ...
                    role=role,
                    field=self.field,
                    processing_kb=proc_kb,
                )
                self.cores.append(core)
                self.role_index[role].append(global_id)
                global_id += 1

            # Report
            sample = self.cores[self.role_index[role][0]]
            total_mb = sample.memory_mb * count
            print(f"    {count} cores × {sample.memory_mb:.1f} MB = {total_mb:.1f} MB")
    ...
    def print_summary(self):
        ...
        print()
        print(f"  Budget (half of 92 GB):   46,000 MB")
        print(f"  Model total:              {total_mb:,.0f} MB ({total_mb/1024:.2f} GB)")
        print(f"  Growth reserve:           {46000 - total_mb:,.0f} MB ({(46000 - total_mb)/1024:.1f} GB)")
        print(f"  Utilization of budget:    {total_mb/46000*100:.1f}%")
        print()
    ...
    def process_signal(self, signal: np.ndarray, target_role: CoreRole = None) -> np.ndarray:
        ...
        if target_role:
            ...
            # Let routers see the signal first
            for r in routers:
                r.process_signal(signal)

        # Process across target cores (round-robin for now)
        results = []
        for i, core in enumerate(cores):
            if i < len(signal) or target_role:
                result = core.process_signal(signal)
                results.append(result)
                # After each core fires, field updates — next core sees it
                break  # Single signal → single core + harmonics
    ...
    def benchmark(self, iterations: int = 5000, signal_size: int = 100) -> dict:
        ...
        print(f"\n  HARMONIC FIELD:")
        print(f"    Active cores: {self.field.active_cores}/{self.TOTAL_CORES}")
